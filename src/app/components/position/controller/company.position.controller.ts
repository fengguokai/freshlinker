...
export class CompanyPositionController {
  ...
  constructor(private candidateService: CandidateService,
    ...
    // this
    var vm = this;
    vm.filter.companyId = $stateParams['companyId'];
    // set breadcrumb
    vm.breadcrumbService.clear();
    vm.breadcrumbService.set({title: 'position.positions', url: '/positions-list'});

    // set page config
    MetaService.pageTitle = vm.$translate.instant('position.position_list');
  ...
  changePosition(index: number) {
    var vm = this;
    vm.firstPosition = angular.copy(vm.positions[index]);
  ...
  collectPosition(id: string) {
    ...
    if (vm.authService.checkUser()) {
      ...
      var item = _.findIndex(vm.bookmark, {id: id});
      if(item === -1) {
